...
int main()
{
    ...
    cout << "len = " << len <<endl;
    cout<<"-----------------"<<endl;
    aux = maxVector(vector1, len);
    cout << "El valor máximo del vector es: " << vector1[aux] << " que se encuentra en la posición: " << aux << endl;
...
int maxVector (int vec[], int len){
   int rdo=0; //la posición 0 la tomo como la pósición en la que se encuentra el max

   for (int i=1; i<len;i++){ //arranco i en 1 porque la posición 0 del vector la asigne como max
      if (vec[i]>vec[rdo]){
         rdo=i;
      }
   }
